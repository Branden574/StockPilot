...
| `delivery-section` | render-only | deep link to SO-000021 (fulfillment=delivery): delivery-request section renders with its action button and the recipients line naming Demo Co's OWN routed recipient (warehouse-demo@…, mig 0337) — and NOT naming dc4@learn4life.org, which would mean the routing read fell back to the compiled L4L pair | #132's SCREEN renders and does not crash. It does NOT verify the compose url or transport — those are unit-pinned; the url handed to the OS is only observable on a tap this read-only suite must not make |
| `size-scan` | render-only | size-count session -> BOTH entry points -> the single-photo scan screen, then the hands-free gate screen | The mobile surface renders and routes, and the hands-free screen degrades to its no-camera branch instead of a black rectangle. A crash on the hands-free route here means **react-native-vision-camera is missing from the installed binary** — the exact regression a JS-only OTA of native work would cause, so this flow is the tripwire for it. It does **NOT** cover capture -> review -> confirm, nor the gate firing: the simulator has no camera, so no frame ever arrives. Those are covered by `apps/web/src/lib/ai/size-scan.test.ts`, the accuracy harness in `apps/web/scripts/size-scan-eval/`, and an end-to-end walk against live prod — **the gate thresholds themselves have never been validated anywhere and need hardware** |
