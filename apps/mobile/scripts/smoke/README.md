...
| `size-scan` | render-only | size-count session -> the "Scan sizes" entry point -> the scan screen behind it | the scanner's mobile surface renders and routes, AND that the removed hands-free entry point stays removed. It does **NOT** cover capture -> review -> confirm: the simulator has no camera, so `takePictureAsync` never returns a frame. That half is unit-pinned in `apps/web/src/lib/ai/size-scan.test.ts`, measured against real photographs by `apps/web/scripts/size-scan-eval/`, and was walked end-to-end against live prod |
