...
export type PendingActionKind =
  ...
  | 'distribute_bundle'
  | 'upload_image'
  // RETIRED 2026-08-24 with the mobile size-count screens. NOTHING ENQUEUES
  // THIS ANY MORE — the kind survives so rows already sitting in an
  // operator's outbox still drain to the server. Deleting it would make those
  // rows fail forever (drainQueue marks an unknown kind 'failed' and re-reads
  // it every tick), silently stranding counts somebody already took.
  | 'size_count_event';
