...
describe('outboxSendDecision', () => {
  ...
  it('nobody signed in: nothing is sent, not even a legacy row', () => {
    expect(outboxSendDecision({ organizationId: null, userId: null }, { orgId: 'org-b', userId: null })).toEqual({
      send: false,
      reason: 'signed-out',
    });
    expect(outboxSendDecision({ organizationId: 'org-a', userId: 'u1' }, { orgId: null, userId: null })).toEqual({
      send: false,
      reason: 'signed-out',
    });
  });
...
describe('the SQL fragments are the same rule as isOwnedBy, for every row and every live user', () => {
  ...
  rowUsers.forEach((u, i) => db.prepare('insert into pending_actions (id, user_id) values (?, ?)').run(i + 1, u));

  const ids = (where: string, live: string | null) =>
    (db.prepare(`select id from pending_actions where ${where} order by id`).all(live) as { id: number }[]).map(
      (r) => r.id,
    );
...
describe('ONE predicate: both drains and every counter use outbox-scope.ts (pattern #26)', () => {
  ...
  it('engine 1 decides per row, inside its loop, with the shared predicate', () => {
    expect(sync).toMatch(/import \{ OutboxSessionChangedError, outboxSendDecision \} from '\.\/outbox-scope'/);
  ...
  it('engine 2 decides per row, inside its loop, with the same predicate', () => {
    expect(engine).toMatch(/outboxSendDecision,\s+REPLACED_BY_LATER_COUNT,\s+\} from '\.\/outbox-scope'/);
    const loop = loopBody(engine, 'for (const row of send)', 'this.pendingCount = await totalPendingCount()');
  ...
  it('every owner filter in SQL is the shared fragment, never a hand-written copy', () => {
    ...
    for (const fn of ['listHeld', 'countHeld']) {
      const body = queue.slice(queue.indexOf(`export async function ${fn}(`)).split('\n}\n')[0] ?? '';
