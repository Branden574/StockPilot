...
export default function ZendeskScreen() {
  ...
  React.useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- fetch-on-mount: every set is post-await except the pre-await error reset and the module-disabled guard that resolves loading; the effect synchronizes with the server
    void load();
  }, [load]);
