...
// ---------------------------------------------------------------------------
// Shipment signed — sent to the recipient (and CC list) when someone signs
// the public /s/[token] page. The signed PDF rides along as an attachment;
// this body is just the cover letter.
// ---------------------------------------------------------------------------

interface ShipmentSignedEmailParams {
  orgName: string;
...
export function shipmentSignedSubject(params: {
  orgName: string;
  workOrderNumber: string;
}): string {
  return `Signed: packing slip ${params.workOrderNumber} — ${params.orgName}`;
}

export function shipmentSignedHtml(params: ShipmentSignedEmailParams): string {
  const {
    orgName,
    workOrderNumber,
    sourceWarehouseName,
  ...
  const dateLabel = SIGNED_DATE_FMT.format(signedAt);
  return `<!doctype html>
<html><body style="margin:0;padding:0;background:#f4f4f5;font-family:-apple-system,Segoe UI,sans-serif;">
  <div style="max-width:560px;margin:40px auto;background:white;border-radius:12px;padding:40px;">
    <div style="display:inline-flex;align-items:center;gap:8px;font-weight:600;font-size:18px;margin-bottom:24px;">
    ...
    <h1 style="font-size:22px;margin:0 0 12px;">Packing slip recorded as signed</h1>
    <p style="color:#52525b;line-height:1.6;margin:0 0 20px;">
      Thanks, ${escapeHtml(signedByName)}. A signed copy of the packing slip is attached for your records.
    </p>
    ...
    <p style="color:#52525b;line-height:1.6;margin:0 0 12px;font-size:13px;">
      Open the attached PDF to view the full packing slip with your signature.
    </p>
    <p style="color:#a1a1aa;font-size:12px;margin-top:32px;border-top:1px solid #e4e4e7;padding-top:16px;">
      Sent by ${escapeHtml(orgName)}.${
        managerName || managerEmail
          ? ` Questions? Contact ${escapeHtml(managerName ?? '')}${managerName && managerEmail ? ' — ' : ''}${managerEmail ? `<a href="mailto:${escapeHtml(managerEmail)}" style="color:#52525b;">${escapeHtml(managerEmail)}</a>` : ''}.`
          : ''
      }
    </p>
  </div>
...
export function shipmentSignedText(params: ShipmentSignedEmailParams): string {
  const {
    orgName,
    workOrderNumber,
    sourceWarehouseName,
  ...
  const dateLabel = SIGNED_DATE_FMT.format(signedAt);
  const lines: string[] = [
    'Packing slip recorded as signed',
    '',
    `Thanks, ${signedByName}. A signed copy of the packing slip is attached for your records.`,
    '',
  ...
  if (managerName || managerEmail) {
    lines.push(
      `Questions? Contact ${managerName ?? ''}${managerName && managerEmail ? ' — ' : ''}${managerEmail ?? ''}`,
    );
  }
  return lines.join('\n');
}
