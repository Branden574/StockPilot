  ...
  if (w === undefined) {
    throw new Error(
      `No ${font} advance width recorded for ${JSON.stringify(char)}. ` +
        'Add it to the metric table in src/lib/pdf/helvetica-metrics.ts before asserting against it.',
    );
  }
  return w;
}
...
/**
 * Non-throwing width for PRODUCTION measurement of arbitrary user values
 * (SKUs, barcodes). Unknown glyphs count as a wide 722/1000 em instead of
 * throwing: over-reserving a column by a few points is harmless, while a
 * thrown error here would 500 the whole export. Tests keep using `width`,
 * which fails loudly so metric-table gaps surface in CI.
 */
export function safeWidth(text: string, font: FontName, sizePt: number): number {
  let total = 0;
  for (const char of text) {
    try {
      total += advance(char, font);
    } catch {
      total += 722;
    }
  }
