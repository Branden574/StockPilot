...
function advance(char: string, font: FontName): number {
  ...
  // would under-measure the content and let a real overflow pass this suite.
  if (w === undefined) {
    throw new Error(
      `No ${font} advance width recorded for ${JSON.stringify(char)}. ` +
        'Add it to the metric table in table-fit.test.ts before asserting against it.',
    );
  }
  return w;
}

/** Width of `text` in points, at `size`pt, ignoring kerning (upper bound). */
function width(text: string, font: FontName, size: number): number {
  let total = 0;
