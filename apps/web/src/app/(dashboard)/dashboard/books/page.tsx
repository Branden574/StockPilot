...
export default async function BooksPage({
  ...
  //
  // Both module checks are module_enabled RPCs and neither reads the
  // other's answer, so they run together: awaited one after the other they
  // put two Supabase round trips in series in front of everything else on
  // this page, and calls from Vercel to Supabase stall 1-8 s at Supabase's
  // entry point on 3-5% of weekday calls (logs, 2026-09-22). The books gate
  // still decides before any book is read: nothing below starts until it
  // has answered. checkModuleAccess resolves (enabled: false) on an RPC
  // error rather than rejecting, and Promise.all observes both either way.
