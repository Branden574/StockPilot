...
// Cached SSR HTML masked the actual root cause — every retry showed
// the same digest because the same baked output was served. Forcing
// dynamic so each request runs fresh.
...
const EVENT_TONE: Array<{ match: (e: string) => boolean; tone: string }> = [
  ...
  {
    match: (e) => e.startsWith('bundle.'),
    tone: 'bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 border-cyan-500/30',
  },
];
...
/**
 * Pulls every input the page needs, swallowing every internal failure
 * and returning either { ok: true, ... } or { ok: false, message }.
 * Putting all the throw sites under one umbrella means any failure —
 * RLS, embed, planner, network — produces the visible red error panel
 * instead of crashing the dashboard error boundary.
 */
async function loadAuditPage(
  searchParams: Promise<{ event?: string; category?: string; actor?: string; cursor?: string }>,
): Promise<{ ok: true; data: AuditFetchResult } | { ok: false; message: string; code: string | null }> {
  try {
    ...
    const userIds = [
      ...new Set(rawRows.map((r) => r.user_id).filter((v): v is string => Boolean(v))),
    ];
    ...
    if (userIds.length > 0) {
      try {
        ...
        // empty map → blank actor
      }
    }

    const rows: AuditRow[] = rawRows
      .map((r) => ({
        id: r.id,
        event: r.event,
    ...
    void reportError(e, { tag: 'audit.page', level: 'error' });
...
export default async function AuditLogPage({
  ...
  const result = await loadAuditPage(searchParams);
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-muted-foreground mt-1 text-[13px]">
                Something went wrong fetching audit data. The error has been
                logged. The actual error message is shown below — please share
                it if the page keeps failing.
              </p>
...
{(result.code ?? 'error') + ': ' + result.message}
  ...
  const { rows, count, prefix, activeCategory, actorTrim, initialActor } = result.data;
