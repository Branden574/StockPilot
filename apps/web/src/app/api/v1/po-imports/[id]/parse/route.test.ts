...
function buildRequest(id = IMPORT_ID) {
  return new Request(`https://test.local/api/v1/po-imports/${id}/parse`, {
    method: 'POST',
...
describe('POST /api/v1/po-imports/[id]/parse', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('re-parses via the service and returns { ok: true }', async () => {
    vi.mocked(withApiContext).mockResolvedValueOnce(buildCtx());
    const parseImport = vi.fn(async () => undefined);
    mockService({ parseImport });
    ...
    expect(parseImport).toHaveBeenCalledWith(IMPORT_ID);
  });
});
