 ...
 * has lapsed it returns `aal2_required`; we surface that with the same toast the
 * rest of the platform console uses (act-as, billing, restore) — the operator
 * re-authenticates with MFA and retries.
 */
export function DeletionPassphraseForm({ isConfigured }: { isConfigured: boolean }) {
  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!canSave) return;
    start(async () => {
      const res = await setOrgDeletionPassphraseAction({
        passphrase,
        currentPassphrase: isConfigured ? current : undefined,
      });
      if (res.ok) {
        toast.success(isConfigured ? 'Org-deletion passphrase rotated.' : 'Org-deletion passphrase set.');
        setCurrent('');
        setPassphrase('');
        setConfirm('');
  ...
  return (
    <div className="rounded-[10px] border border-destructive/40 bg-card p-4 sm:p-5">
      <div className="flex items-center gap-2">
        <KeyRound className="h-4 w-4 text-destructive" strokeWidth={1.75} />
      ...
      <p className="mt-1.5 max-w-2xl text-[12.5px] text-[var(--ed-ink-3)]">
        ...
        organization — on top of the platform-admin allowlist and a fresh MFA step-up. It is stored
        hashed (scrypt) and is <span className="font-medium">never retrievable</span>; if it is lost,
        rotate it here. Saving requires a fresh MFA step-up and is recorded in the platform audit log.
      </p>
      ...
      <form onSubmit={onSubmit} className="mt-4 max-w-md space-y-4">
        ...
        <div className="space-y-1.5">
          <Label htmlFor="deletion-passphrase">{isConfigured ? 'New passphrase' : 'Passphrase'}</Label>
          <PasswordInput
          ...
          />
          {tooShort ? (
            <p className="text-[11.5px] text-destructive">Must be at least 12 characters.</p>
          ) : null}
        </div>
        ...
        <div className="space-y-1.5">
          ...
          />
          {mismatch ? (
            <p className="text-[11.5px] text-destructive">Passphrases don&apos;t match.</p>
          ) : null}
        </div>
