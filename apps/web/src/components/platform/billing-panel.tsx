...
export function BillingPanel(props: BillingPanelProps) {
  ...
  function onStartTrial() {
    ...
    startTrialTx(async () => {
      ...
      let res = await startOrgTrialAction(payload);
      // Fresh MFA step-up needed → prompt for the authenticator code in-place
      // (no sign-out) and retry with the identical payload.
      if (!res.ok && aal2(res.error.details)) {
        if (!(await stepUp.ensure())) return;
        res = await startOrgTrialAction(payload);
      }
  ...
  return (
    <>
      ...
      <div className="space-y-6">
        ...
        {/* ── Override panel ───────────────────────────────────────────── */}
        <div className="border-border space-y-4 rounded-[10px] border p-4">
          ...
          <div className="space-y-1">
            ...
            <textarea
              ...
              className="border-border bg-background w-full rounded-md border px-2.5 py-2 text-[13px] outline-none focus:border-[var(--ed-line-strong)]"
            />
          </div>
          ...
          <button
            ...
            onClick={onSave}
            disabled={saving}
            className="bg-foreground text-background rounded-md px-3 py-1.5 text-[13px] font-medium disabled:opacity-50"
          >
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            ...
            <div className="flex items-end">
              <button
                ...
                disabled={trialing}
                className="border-border h-9 rounded-md border px-3 text-[13px] font-medium hover:border-[var(--ed-line-strong)] disabled:opacity-50"
              >
