...
describe('buildDeliveryRequestDraft — block order is a truncation contract', () => {
  ...
  it('puts the order handle, the destination and the needed-by BEFORE the item rows', () => {
    ...
    expect(at.handle).toBeLessThan(at.destination);
  ...
  it('holds the same order on a pickup draft, where the destination block is a handoff', () => {
    ...
    expect(body.indexOf('Order: SO-000049')).toBeLessThan(body.indexOf('PICKUP FROM'));
