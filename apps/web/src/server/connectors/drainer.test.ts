...
function makeFakeAdmin(opts: {
  ...
  function builder(table: string) {
    ...
    const self: any = {
      ...
      limit() {
        // terminal for the events select
        if (table === 'outbox_events') {
          return Promise.resolve({ data: opts.events ?? [], error: err('outbox_events:select') });
        }
        return self;
      },
      maybeSingle() {
...
describe('runDrain error handling (fail-closed)', () => {
  ...
  it('throws when the outbox_events select fails', async () => {
    const admin = makeFakeAdmin({
      ...
      errors: { 'outbox_events:select': 'events boom' },
    });
    await expect(
