...
--
-- Routes PO receiving accepted qty into the warehouse Staging location.
-- Accepted stock lands in item_stock_levels[item, Staging] and quantity_on_hand
-- still rises (total owned). Operators can then place items from Staging into
-- racks/crates in a later step.
--
-- Based on the authoritative 0069 body (preserves: over-receive guard, negative-qty
-- rejection, lot/serial validation + receipt_line_lots/serial_registry persistence,
-- idempotency, recompute_po_status). Also preserves the 0134 fix of
-- set search_path = public, extensions so pgcrypto's digest() resolves.
--
-- Diff vs 0069 (exactly 4 surgical deltas, nothing else):
--   (a) search_path = public, extensions  (restores 0134 fix — prevents "function
--       digest(text, unknown) does not exist" on Supabase where pgcrypto is in
--       the extensions schema)
--   (b) v_staging uuid;  added to DECLARE block
--   (c) ensure_warehouse_placement_locations + staging-id SELECT added after the
--       has_org_role / po_already_closed guards and before the receipts INSERT
--   (d) null location arg in adjust_stock call replaced with v_staging
  ...
  for v_line in select * from jsonb_to_recordset(p_lines) as x(
    ...
    if v_line.qty_accepted > 0 then
      perform public.adjust_stock(
        ...
        v_line.qty_accepted,
        'receive_po',
        v_staging,       -- route accepted qty into the warehouse Staging location (was null)
        'receipt_line',
        v_receipt.id::text
      );
    end if;
