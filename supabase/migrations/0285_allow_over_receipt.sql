-- Allow over-receipt on POs (owner decision 2026-07-21).
--
-- Vendors sometimes ship MORE than was ordered. The original post_receipt_v2
-- (rule from 0069) hard-blocked any receipt whose cumulative accepted qty
-- exceeded the ordered qty, raising `over_receive_blocked` — which the service
-- didn't map to a friendly code, so warehouse staff saw only a generic
-- "An internal error occurred." Owner: receiving extras is legitimate and the
-- receipt Notes field already lets staff record why (e.g. "vendor sent 9 extra").
--
-- This migration re-creates post_receipt_v2 IDENTICALLY except it drops the
-- over-receive guard. recompute_po_status already treats received >= ordered as
-- 'received', so an over-received line closes the PO correctly and the line
-- variance simply shows negative ("N over"). All other validations
-- (non-negative quantities, lot/serial integrity, idempotency, permission,
-- PO-open state) are unchanged.
