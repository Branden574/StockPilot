...
**CORRECTION (2026-08-02, owner decision).** The subject line described above — `Delivery Request — StockPilot Order <handle> — <site or warehouse>` — no longer carries the order handle. The owner cut the middle segment for a simpler subject: `Delivery Request — <site or warehouse>` (the em dash separator is unchanged). The order number is not lost — it remains the body's first content line (`Order: SO-000049`), and the condensed-mode disclosure ("...under the order number above") stays literally true because that line is still there. The owner acknowledged, and accepted, that two deliveries to the same site now produce byte-identical subjects: Zendesk creates one ticket per inbound email regardless of subject text, and mail-client threading groups by `In-Reply-To`/`References` headers, not by subject string, so nothing downstream relies on the subject being unique per order. See `buildDeliveryRequestDraft` in `storefront-logic.ts` and the `buildDeliveryRequestDraft — subject` describe block in `storefront-logic.test.ts` for the current shape and its test coverage.
...
**CORRECTION (2026-08-02, owner decision).** The body's `Order status in StockPilot: Pending approval` line, and the `DRAFT_STATUS_LABEL` constant that produced it, are gone. That line was a MOUNT-TIME SNAPSHOT — accurate the instant the draft opened, but capable of reading stale by the time DC4 actually opens the email if the order's real status had changed in the interim (e.g., approved) — and the owner simplified rather than caveated it: the line was redundant, since the audit event and the order-timeline entry described earlier in this section already record that a draft was opened, and the order's real-time status is one click away inside StockPilot for anyone who needs it. The order's status is not otherwise asserted anywhere in the email. Any staleness watch-for previously flagged for `DRAFT_STATUS_LABEL` is now moot — there is no longer a status claim in the body that can go stale. Everything else about this section (the audit event, its allow-list, and the re-verified success-screen sentence) is unaffected.
